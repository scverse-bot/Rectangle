...
def test_pseudo_bulk_creation_anndata():
    group_size = 3
    sc_data = pd.read_csv("./data/sc_object_small.csv", index_col=0)
    annotations = np.array(list(pd.read_csv("./data/cell_annotations_small.txt", header=None, index_col=0).index))
